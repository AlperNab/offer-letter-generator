...
**Domain:** `HR / Legal Docs` · **Suite:** `HR & Recruiting Suite` · **Accent:** `#fbbf24`
...
**Offer Letter Generator** is a standalone, browser-based AI workflow app for **HR / Legal Docs**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
| `compensation`<br>compensation | text | Yes | — | Affects input: compensation. |
...
| `employment_type`<br>employment type | text | No | — | Affects customization: employment type. |
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `benefits_library`<br>benefits library | text | No | — | Affects customization: benefits library. |
| `tone`<br>tone | text | No | — | Affects customization: tone. |
| `legal_strictness`<br>legal strictness | slider | No | 50 | Affects customization: legal strictness. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9141
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9141`. |
...
```bash
curl http://127.0.0.1:9141/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9141/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9141 uvicorn app.main:app --host 0.0.0.0 --port 9141
```
...
- `plugins/offer-letter-generator.json` for plugin metadata.
